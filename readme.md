...
![alt text](.assets/image1011a.png)

Програма створює новий (дочірній) процес за допомогою функції fork(). Якщо fork() завершується з помилкою, програма виводить повідомлення про помилку. У дочірньому процесі виводяться його власний PID (ідентифікатор процесу) та PID батьківського процесу. У той час батьківський процес чекає завершення дочірнього, використовуючи wait(NULL).
...
![alt text](.assets/image1213a.png)

Програма створює дочірній процес за допомогою fork(), у якому виконується потенційно аварійна програма ./test через execl(). Батьківський процес чекає завершення дочірнього за допомогою waitpid(), і якщо той завершився аварійно (тобто отримав сигнал SIGSEGV, SIGFPE, SIGILL тощо), то батько записує інформацію про час, PID, PPID та тип сигналу у базу даних SQLite. У такий спосіб програма дозволяє відстежувати крахи підлеглих процесів і зберігати журнал цих аварій.

# Практична 14
## Завдання (варіант 2)
Реалізуйте два незалежних POSIX таймери, які працюють на різних інтервалах та логують в окремі файли.

![alt text](.assets/image1411a.png)
log1 (кожні 2 секунди):
![alt text](.assets/image1421a.png)
log2 (кожні 5 секунд):
![alt text](.assets/image1431a.png)

Ця програма створює два незалежних POSIX-таймери, кожен з яких працює з власним інтервалом та сигналом (SIGRTMIN і SIGRTMIN+1). Коли кожен із таймерів спрацьовує, відповідний обробник сигналу викликається та записує повідомлення з міткою часу у свій окремий лог-файл (log1.txt для першого таймера та log2.txt для другого). Один таймер активується кожні 2 секунди, інший — кожні 5 секунд. Програма працює безкінечно, чекаючи сигнали, і таким чином дозволяє одночасно відстежувати події на різних інтервалах часу з точним логуванням у реальному часі.
